    ...
    <div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 15, color: colors.textMuted, margin: 0 }}>{eleves.length} eleve{eleves.length > 1 ? "s" : ""} inscrits</p>
            </div>
          </div>
          <div style={{ display: "flex", gap: 12 }}>
            <button
              onClick={async () => {
                setExporting(true);
                try {
                  const res = await exportElevesExcelSecure({ classe: filterClasse || undefined });
              ...
              disabled={exporting}
              style={{ padding: "12px 20px", background: colors.bgSecondary, color: colors.textSecondary, borderRadius: 10, border: "none", fontSize: 14, fontWeight: 500, display: "flex", alignItems: "center", gap: 8, cursor: exporting ? "not-allowed" : "pointer", opacity: exporting ? 0.7 : 1 }}
            >
            ...
            <Link to="/eleves/nouveau" style={{ padding: "12px 20px", background: colors.gradientPrimary, color: colors.onGradient, borderRadius: 10, textDecoration: "none", fontSize: 14, fontWeight: 500, display: "flex", alignItems: "center", gap: 8, boxShadow: colors.shadowPrimary }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(classStats.length, 5)}, 1fr)`, gap: 12, marginBottom: 24 }}>
          {classStats.map((stat) => (
            <button
              ...
              style={{
                background: filterClasse === stat.classe ? colors.primary : colors.bgCard,
                borderRadius: 12,
                padding: 16,
                border: filterClasse === stat.classe ? `2px solid ${colors.primary}` : `1px solid ${colors.border}`,
                cursor: "pointer",
                textAlign: "left"
              }}
            >
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24, alignItems: "center" }}>
        <input
          ...
          style={{ flex: 1, padding: "12px 16px", border: `1px solid ${colors.border}`, borderRadius: 10, fontSize: 14, outline: "none", background: colors.bgCard, color: colors.text }}
        />
        <select
          value={filterClasse}
          onChange={(e) => setFilterClasse(e.target.value)}
          aria-label="Filtrer par classe"
          style={{ padding: "12px 16px", border: `1px solid ${colors.border}`, borderRadius: 10, fontSize: 14, background: colors.bgCard, minWidth: 160, color: colors.text }}
        >
        ...
        <div style={{ display: "flex", background: colors.bgSecondary, borderRadius: 8, padding: 4 }}>
          <button
            onClick={() => setViewMode("all")}
            style={{ padding: "8px 16px", background: viewMode === "all" ? colors.bgCard : "transparent", color: viewMode === "all" ? colors.text : colors.textMuted, border: "none", borderRadius: 6, fontSize: 13, fontWeight: 500, cursor: "pointer", boxShadow: viewMode === "all" ? "0 1px 2px rgba(0,0,0,0.05)" : "none" }}
          >
            Liste
          </button>
          <button
            onClick={() => setViewMode("byClass")}
            style={{ padding: "8px 16px", background: viewMode === "byClass" ? colors.bgCard : "transparent", color: viewMode === "byClass" ? colors.text : colors.textMuted, border: "none", borderRadius: 6, fontSize: 13, fontWeight: 500, cursor: "pointer", boxShadow: viewMode === "byClass" ? "0 1px 2px rgba(0,0,0,0.05)" : "none" }}
          >
            Par classe
          </button>
        </div>
              ...
              <div key={classe} style={{ background: colors.bgCard, borderRadius: 16, border: `1px solid ${colors.border}`, overflow: "hidden" }}>
                <div style={{ padding: "16px 20px", borderBottom: `1px solid ${colors.border}`, background: colors.bg, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      <p style={{ margin: 0, fontWeight: 600, color: colors.text }}>{classe}</p>
                      <p style={{ margin: 0, fontSize: 12, color: colors.textMuted }}>{classEleves.length} eleve{classEleves.length > 1 ? "s" : ""}</p>
                    </div>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, padding: 16 }}>
                  {classEleves.map((eleve) => (
                    <Link key={eleve.id} to={`/eleves/${eleve.id}`} style={{ display: "flex", alignItems: "center", gap: 12, padding: 12, background: colors.bg, borderRadius: 10, textDecoration: "none" }}>
                      <div style={{ width: 36, height: 36, borderRadius: 8, background: eleve.sexe === "M" ? colors.maleBg : colors.femaleBg, color: eleve.sexe === "M" ? colors.maleText : colors.femaleText, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 600, fontSize: 12 }}>
                        {eleve.prenom[0]}{eleve.nom[0]}
                      </div>
                      <div>
                        <p style={{ margin: 0, fontWeight: 500, color: colors.text, fontSize: 14 }}>{eleve.prenom} {eleve.nom}</p>
                        <p style={{ margin: 0, fontSize: 11, color: colors.textLight }}>{eleve.sexe === "M" ? "Garcon" : "Fille"}</p>
                      </div>
                    </Link>
                  ))}
                </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: colors.bg }}>
                ...
                <th style={{ padding: "14px 20px", textAlign: "left", fontSize: 12, fontWeight: 600, color: colors.textMuted, textTransform: "uppercase", letterSpacing: "0.5px" }}>Sexe</th>
                <th style={{ padding: "14px 20px", textAlign: "left", fontSize: 12, fontWeight: 600, color: colors.textMuted, textTransform: "uppercase", letterSpacing: "0.5px" }}>Statut</th>
            ...
            <tbody>
              {filtered.map((eleve) => (
                <tr key={eleve.id} style={{ borderTop: `1px solid ${colors.border}` }}>
                  <td style={{ padding: "16px 20px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: eleve.sexe === "M" ? colors.maleBg : colors.femaleBg, color: eleve.sexe === "M" ? colors.maleText : colors.femaleText, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 600, fontSize: 14 }}>
                        {eleve.prenom[0]}{eleve.nom[0]}
                      </div>
                      <div>
                  ...
                  <td style={{ padding: "16px 20px" }}>
                    <span style={{ padding: "4px 12px", background: colors.bgSecondary, borderRadius: 6, fontSize: 13, color: colors.textSecondary }}>
                      {eleve.classe}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: "16px 20px" }}>
                    <span style={{ padding: "4px 12px", background: eleve.statut === "actif" ? colors.successBg : colors.dangerBg, color: eleve.statut === "actif" ? colors.success : colors.danger, borderRadius: 20, fontSize: 12, fontWeight: 500 }}>
                      {eleve.statut === "actif" ? "Actif" : "Inactif"}
                    </span>
                  </td>
                  <td style={{ padding: "16px 20px", textAlign: "right" }}>
                    <Link to={`/eleves/${eleve.id}`} style={{ padding: "8px 16px", background: colors.bgSecondary, color: colors.textSecondary, borderRadius: 8, textDecoration: "none", fontSize: 13, fontWeight: 500 }}>
                      Voir
                    </Link>
                  </td>
                </tr>
